...
import { formatCount, formatMs } from './format.ts';
...
export function metricOf(id: string): MetricSpec {
  return BY_ID.get(id) || METRICS[0];
}

export function formatMetric(id: string, value: number | null | undefined): string {
...
const VIEW_TO_RANGE: Record<string, string> = {
  daily: 'day',
  weekly: 'week',
  monthly: 'month',
  yearly: 'year',
...
export function rangeFromDefaultView(view: string): string {
  return VIEW_TO_RANGE[view] || 'day';
}

/** 键盘时间分布的四个视图（services/keyboard.py 的 TIMELINE_VIEWS）。 */
export const TIMELINE_VIEWS: readonly RangeSpec[] = [
  { id: 'hours', name: '时' },
  { id: 'days', name: '日' },
  { id: 'months', name: '月' },
  { id: 'years', name: '年' },
...
export const PROFILES = ['input_heavy', 'interactive', 'passive', 'idle_open'] as const;
 ...
 */
export function heatLevel(ratio: number): number {
  if (ratio <= 0) return 0;
...
export const HEAT_BOUNDS = [0, 0.2, 0.4, 0.6, 0.8, 1] as const;
