...
//
// **这里是整个前端唯一把未校验 JSON 认作声明形状的地方**。`fetchInto` 按 `DataMap`
// 把 key 映射到响应类型，视图因此不必在每个使用点断言。那一次断言不是凭空的：
// tests/integration/test_frontend_contract.py 拿真实端点的响应逐层核对
// types/api.d.ts，字段改名或消失都会红。
import { ApiError, get } from './api.ts';
import type { Params } from './api.ts';
import { setEntry, setState } from './store.ts';
import type { DataMap, DataRequest, RequestFailure } from '../types/api.d.ts';
 ...
 */
export async function fetchInto<K extends keyof DataMap>(
  key: K,
  path: string,
  params: Params = {},
  options: { maxAge?: number } = {},
): Promise<DataMap[K] | null> {
  const { maxAge = 0 } = options;
  setEntry('loading', key, true);
  const signal = controller.signal;
  try {
    const payload = (await get(path, params, { signal, maxAge })) as DataMap[K] | null;
    ...
    if (payload && 'coverage' in payload && payload.coverage) {
      setState('coverage', payload.coverage);
    }
    return payload;
  } catch (error) {
    if ((error as { name?: string } | null)?.name === 'AbortError' || signal.aborted) return null;
...
function describe(error: unknown): RequestFailure {
 ...
 */
export function fetchAll(requests: readonly DataRequest[]): Promise<unknown[]> {
  return Promise.all(
    requests.map(({ key, path, params, options }) => fetchInto(key, path, params, options)),
  );
}
