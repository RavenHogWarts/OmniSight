...
两者的差别只有**安装位置**，功能完全一样，只有一处例外：设置页的「登录时以管理员身份启动」一般只有安装版能开——它建的登录任务必须指向一个**普通用户改不动的 EXE**，而判定看的是那个目录的权限、不是路径（装到哪个盘都行，理由见 [docs/faq.md](docs/faq.md)）。安装版默认装进 `Program Files`（安装过程需要一次管理员确认）；便携版解压到任意**可写**目录（桌面、文档、U 盘都行，别放 `Program Files` 那类需要管理员权限的地方）。
...
─────────────
暂停记录                  勾选态，暂停时图标变灰
以管理员身份重启          仅当需要统计管理员程序里的按键时才用，见下
─────────────
开机自启                  勾选态
打开数据目录
打开日志目录
...
**以管理员身份重启**：Windows 不允许普通权限的程序收到发往更高权限程序的键盘输入。如果你有些软件是以管理员身份运行的（管理员模式的 VS Code、终端、任务管理器），它们里面敲的键**统计不到**——症状是那个应用的按键数一直是 0，而屏幕时间照常记录。这一项让 OmniSight 提权重启（会弹一次 UAC 确认框）来补上这个缺口。

它**只对本次运行有效**：下次启动回到普通权限，需要时再点一次。这是有意的——"每次开机都自动以管理员身份运行一个记录键盘的程序"是个大得多的决定，不该由一个菜单项顺带做掉。日常使用**不需要**管理员权限，详见 [docs/faq.md](docs/faq.md)。
