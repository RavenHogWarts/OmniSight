 ...
 * 没有响应时退回本地今天。
 */
export function shift(range, anchor, direction) {
  ...
  switch (range) {
    ...
    default: return base;   // total / custom 没有"上一个"
  }
}

/** 能不能往后翻。后端会把未来截断，但按钮该先置灰，而不是点了没反应。 */
export function canGoForward(range, anchor, today = todayISO()) {
  if (range === 'total' || range === 'custom') return false;
  const next = shift(range, anchor, 1);
  if (range === 'day') return next <= today;
  // 周/月/年：只要下一个周期的锚点不晚于今天就还有得看。
  return next <= today;
}

export function isPageable(range) {
  return range !== 'total' && range !== 'custom';
}

/** 键盘时间分布的桶数（画骨架屏时要知道画几格）。 */
export function bucketCountHint(view) {
  switch (view) {
 ...
 */
export function gapSet(coverage, kinds = ['foreground']) {
...
export function caliberNotes(coverage) {
  return (coverage?.gaps || [])
    .filter((gap) => gap?.missing === 'key_position')
    .map((gap) => ({ from: gap.from, to: gap.to, message: gap.message, reason: gap.reason }));
}

/** 桶 id -> 是否命中缺口。日桶精确匹配，月/年桶只要包含一天缺口就算。 */
export function bucketCoversGap(bucket, gaps) {
