...
// （不补零）。tests/frontend/format.test.js 用与 tests/unit/test_formatting.py 相同的
// 表格固定这些值，两边同时改才可能改。
...
export function formatClockRange(startIso, endIso) {
  const start = formatClock(startIso);
  const end = formatClock(endIso);
  return start && end ? `${start}\u2013${end}` : start || end;
}

/** `2026-09-02` -> `9月2日`。周期标题一律用后端给的 period.label，这个只给轴用。 */
